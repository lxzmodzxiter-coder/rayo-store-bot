...
async def main():
    ...
    async with async_session_maker() as session:
        ...
        await cmd_rol(FakeMessage(owner_tg, "/rol 999 vendedor"), session, current_user=owner)
        await session.refresh(user)
        assert user.role == UserRole.SELLER
        assert is_staff(user) and not is_admin(user)

        await cmd_rango(FakeMessage(owner_tg, "/rango 999 Socio Elite"), session, current_user=owner)
        await session.refresh(user)
        assert user.rank_title == "Socio Elite"

        await cmd_ban(FakeMessage(owner_tg, "/ban 999 prueba"), session, current_user=owner)
        await session.refresh(user)
        ...
        assert "protegido" in protected.answers[0]
